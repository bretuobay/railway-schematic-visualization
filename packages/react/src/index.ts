...
export interface RailSchematicRenderer {
  getSVGString(): string;
  render(snapshot: RailSchematicRenderSnapshot): void;
...
export interface ReactRailSchematicResources {
  readonly renderer: RailSchematicRenderer;
  readonly viewport: RailSchematicViewport;
  readonly overlayManager: RailSchematicOverlayManager;
  readonly exportSystem: ExportSystem;
}

export interface ReactLifecycleInput {
  readonly data?: RailGraph | null | undefined;
  readonly width?: number | string | undefined;
  readonly height?: number | string | undefined;
  readonly layoutMode?: string | undefined;
  readonly style?: Readonly<Record<string, unknown>> | undefined;
  readonly overlays?: ReadonlyArray<RailSchematicOverlay> | undefined;
  readonly viewport?: Partial<RailSchematicViewportState> | undefined;
}

export type ReactLifecycleUpdate = ReactLifecycleInput;

export type ReactLifecycleFactories = LifecycleFactorySet<
  ReactLifecycleInput,
...
export interface RailSchematicProps extends ReactLifecycleInput {
  ...
  readonly lifecycleFactories?: ReactLifecycleFactories;
}

export interface RailSchematicRef {
  ...
  clearSelection(): void;
  getRenderer(): RailSchematicRenderer;
  getViewport(): RailSchematicViewport;
  getOverlayManager(): RailSchematicOverlayManager;
  getExportSystem(): ExportSystem;
}

export interface UseRailSchematicResult {
  readonly viewport: {
    readonly position: {
      readonly x: number;
      readonly y: number;
  ...
  readonly overlays: {
    add(overlay: RailSchematicOverlay): string;
...
function createDefaultLifecycleFactories(
  ...
  function ensureStore(input: ReactLifecycleInput): DefaultStore {
    if (!storeRef.current) {
      storeRef.current = {
  ...
  function resolveGraph(data: RailGraph | null | undefined): RailGraph {
    ...
    if (!(data instanceof RailGraph)) {
      throw new TypeError('RailSchematic adapters require the data prop to be a RailGraph instance.');
    }
  ...
  const overlayManager: RailSchematicOverlayManager = {
    ...
    addOverlay: (overlay) => {
      ...
      if (!overlayRecords.has(overlayId)) {
        const adapterOverlay = new AdapterOverlay(overlayId, overlay.visible ?? true);
    ...
    sync: (overlays) => {
      ...
      for (const overlay of normalized) {
        if (!overlayRecords.has(overlay.id)) {
          const adapterOverlay = new AdapterOverlay(overlay.id, overlay.visible);
...
interface RailSchematicRenderSnapshot extends ReactLifecycleInput {}
...
function bindMappedHandlers(
  ...
  for (const [eventName, handler] of handlerMap) {
    ...
    const wrapped = (payload: Readonly<Record<string, unknown>>) => {
      handler(eventMapper.mapPayload(eventName, payload));
    };
  ...
  if (props.onViewportChange) {
    const wrappedViewport = (payload: Readonly<Record<string, unknown>>) => {
      props.onViewportChange?.(eventMapper.mapPayload('viewport-change', payload));
    };
...
function createLifecycleInput(props: RailSchematicProps): ReactLifecycleInput {
  return {
    data: props.data,
    width: props.width,
    height: props.height,
...
export const REACT_ADAPTER_METADATA = {
  framework: 'react',
  packageName: '@rail-schematic-viz/react',
  version: '0.1.0',
} as const satisfies AdapterPackageMetadata;

export const REACT_ADAPTER_SURFACE = {
  framework: 'react',
  capabilities: {
    svg: true,
    png: true,
...
export const RailSchematic = memo(
  forwardRef<RailSchematicRef, RailSchematicProps>(function RailSchematicComponent(
    props,
    ref,
  ) {
    const storeRef = useRef<DefaultStore | undefined>(undefined);
    const initializationErrorRef = useRef<Error | undefined>(undefined);
    const eventMapper = useMemo(() => new EventMapper(), []);
    ...
    >(
      new LifecycleManager(
        props.lifecycleFactories ?? createDefaultLifecycleFactories(storeRef),
      ),
    );
    ...
    useEffect(() => {
      ...
      void lifecycleRef.current
        .initialize(createLifecycleInput(props))
        .then((resources) => {
          if (!mounted) {
            return;
          }

          initializationErrorRef.current = undefined;
          eventCleanupRef.current?.();
          eventCleanupRef.current = bindMappedHandlers(resources, props, eventMapper);
        })
        .catch((error: unknown) => {
          initializationErrorRef.current = error instanceof Error
            ? error
    ...
    useEffect(() => {
      const resources = lifecycleRef.current.getResources();

      if (!resources) {
        return;
      }

      void lifecycleRef.current.update(createLifecycleInput(props));
      eventCleanupRef.current?.();
      eventCleanupRef.current = bindMappedHandlers(resources, props, eventMapper);
    }, [
      props.data,
    ...
    useImperativeHandle(ref, () => {
      const requireResources = (): ReactRailSchematicResources => {
        ...
        if (!resources || !resources.exportSystem) {
          throw new Error('RailSchematic is not initialized.');
        }
        ...
        return {
          ...
          exportSystem: resources.exportSystem,
        };
      };
      ...
      return {
        ...
        selectElements: (ids) => {
          ...
          if (storeRef.current) {
            ...
            resources.renderer.render(createLifecycleInput(props));
          }
...
export function useRailSchematic(
  componentRef: { readonly current: RailSchematicRef | null },
): UseRailSchematicResult {
  const viewportSnapshotRef = useRef<RailSchematicViewportState>({
    x: 0,
    y: 0,
    scale: 1,
  });
  const selectionSnapshotRef = useRef<ReadonlyArray<string>>([]);
  ...
  useEffect(() => {
    ...
    const renderer = component.getRenderer() as RailSchematicRenderer & {
      on?: (event: string, handler: RailSchematicEventHandler) => void;
      off?: (event: string, handler: RailSchematicEventHandler) => void;
    };
    ...
    const onSelectionChange = (payload: Readonly<Record<string, unknown>>) => {
      if (Array.isArray(payload.selection)) {
        selectionSnapshotRef.current = payload.selection.filter(
          (value): value is string => typeof value === 'string',
        );
      }
    };
    const onViewportChange = (payload: Readonly<Record<string, unknown>>) => {
      const transform = payload.transform as Partial<RailSchematicViewportState> | undefined;
      ...
      viewportSnapshotRef.current = {
        x: transform.x ?? viewportSnapshotRef.current.x,
        y: transform.y ?? viewportSnapshotRef.current.y,
        scale: transform.scale ?? viewportSnapshotRef.current.scale,
      };
    };

    viewportSnapshotRef.current = viewport.getTransform();
  ...
  return useMemo<UseRailSchematicResult>(() => ({
    viewport: {
      get position() {
        return {
          x: viewportSnapshotRef.current.x,
          y: viewportSnapshotRef.current.y,
        };
      },
      get scale() {
        return viewportSnapshotRef.current.scale;
      },
      pan: async (x, y) => {
